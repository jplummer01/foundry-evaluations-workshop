...
- **Same evaluators, held constant** — run candidate vs. incumbent on the *same* dataset with the *same* judge model, and compare **deltas, not absolute scores**. A fixed judge across comparisons is essential; changing the judge mid-migration invalidates the comparison.
- **Statistical rigour comes free in CI**: the Azure DevOps / GitHub Actions evaluation tasks report per-metric confidence intervals and a **pairwise statistical comparison** across agent/model versions — precisely the "is this difference meaningful or noise?" question a migration decision hinges on (see `examples/`).
...
Because trace-based evaluation is framework- and cloud-agnostic, this works even when the candidate runs on different serving infrastructure than the incumbent.

### 5–6. Canary with evaluation-driven rollback, then keep watching

Ramp 1% → 5% → 25% → 100%, segmented by cohort. Wire Azure Monitor alerts on:
...
Post-migration, continuous evaluation keeps running — the new model becomes the incumbent baseline for the *next* migration.

## Prompts migrate too

Prompts are usually overfit to a specific model's quirks. Budget for **prompt re-tuning as part of every migration**, and version prompts alongside models so an eval result is always attributable to a (model version, prompt version) pair. Keep prompts model-agnostic where feasible, but expect the eval harness — not hope — to tell you when re-tuning is done.
...
- [ ] Shadow-traffic diffs reviewed, divergent-row judge reasoning read
...
- [ ] Comparison documented as an artifact for audit/governance — Foundry evaluation runs persist in the project, which gives you this for free
