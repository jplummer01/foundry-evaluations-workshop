"""Create the GMP discipline agent: a manufacturing SOP & deviation-triage assistant.

One of four GxP discipline tracks (see docs/gxp-disciplines.md). This is the
**GMP - Good Manufacturing Practice** track: an agent for a pharmaceutical
manufacturing site that answers procedural questions grounded in mock SOP
excerpts via a lookup_sop tool, helps triage deviations, and must REFUSE
decision-shaped and data-integrity-violating requests. It is the named GMP
version of the generic SOP agent in create_agent_gxp.py; the generic file is
retained as the intro/reference example.

The refusal behaviours (batch release, change-control shortcuts, ALCOA+ record
alteration) are what the decision_refusal / data_integrity_trap / prompt_injection
rows in dataset_gmp_sample.jsonl evaluate.

SDK-drift risk: create_version / definition shape targets the 2.x
azure-ai-projects GA surface (July 2026 baseline). If a call signature has
moved, keep the shape: agent definition -> create_version.
...
AGENT_NAME = os.environ.get("GMP_AGENT_NAME", "demo-gmp-agent")
...
def main() -> None:
    ...
    print(f"  GMP_AGENT_NAME={AGENT_NAME}")
    print(f"  GMP_AGENT_VERSION={agent.version}")
    print("\nExecutable smoke test (the portal cannot run this local Python tool):")
    print("  python run_agent.py --agent-name demo-gmp-agent --query")
    print("    \"what goes in a cleaning logbook entry?\"")
